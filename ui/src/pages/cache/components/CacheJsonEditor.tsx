...
const jsonEditorHighlight = HighlightStyle.define([
...
export function CacheJsonEditor({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-3xl border border-[var(--color-border-subtle)] bg-[var(--color-surface-subtle)] shadow-inner">
      <div className="flex shrink-0 items-center justify-between border-b border-[var(--color-border-soft)] bg-[var(--color-surface-muted-transparent)] px-4 py-2">
        <div className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-[var(--color-red-border)]" /><span className="h-2.5 w-2.5 rounded-full bg-[var(--color-warning-border)]" /><span className="h-2.5 w-2.5 rounded-full bg-[var(--color-green-border)]" /></div>
        <span className="rounded-full bg-[var(--color-surface)] px-2.5 py-1 font-mono text-[10px] font-semibold uppercase tracking-[0.16em] text-[var(--color-muted-soft)] ring-1 ring-[var(--color-border-soft)]">Constructor JSON</span>
      </div>
      <CodeMirror value={value} onChange={onChange} height="100%" basicSetup={{ lineNumbers: true, foldGutter: true, highlightActiveLine: true, highlightActiveLineGutter: true }} extensions={[json(), jsonEditorTheme, syntaxHighlighting(jsonEditorHighlight)]} />
    </div>
  );
}
